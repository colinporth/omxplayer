...
void SubtitleRenderer::load_glyph (InternalChar ch) {
  ...
  //{{{
  auto load_glyph_internal = [&](FT_Face ft_face, VGFont vg_font, bool border) {
    try {
      ...
      if (bitmap.width > 0 && bitmap.rows > 0) {
        ...
        auto softened_image = vgCreateImage(VG_A_8, image_width, image_height, VG_IMAGE_QUALITY_NONANTIALIASED);
...
//}}}
//{{{
int SubtitleRenderer::get_text_width (const vector<InternalChar>& text) {
...
//}}}
//{{{
void SubtitleRenderer::prepare_glyphs (const vector<InternalChar>& text) {
...
//}}}
//{{{
void SubtitleRenderer::draw_text (VGFont font, const vector<SubtitleRenderer::InternalChar>& text,
...
SubtitleRenderer:: SubtitleRenderer (int display, int layer,
                                     ...
                                     float margin_left, float margin_bottom, bool centered,
                                     unsigned int white_level, unsigned int box_opacity, unsigned int lines)
    : prepared_(), dispman_element_(), dispman_display_(),
      display_(), context_(), surface_(), vg_font_(), vg_font_border_(),
      ft_library_(), ft_face_(), ft_face_italic_(), ft_stroker_(),
      centered_(centered), white_level_(white_level), box_opacity_(box_opacity), font_size_(font_size) {

  try {
    ENFORCE(graphics_get_display_size(display, &screen_width_, &screen_height_) >= 0);
    initialize_fonts(font_path, italic_font_path);

    int abs_margin_bottom = static_cast<int>(margin_bottom * screen_height_ + 0.5f) - config_.box_offset;
...
void SubtitleRenderer::initialize_window (int display, int layer) {
...

  {
    ...
    dispman_element_ = vc_dispmanx_element_add (dispman_update, dispman_display_, layer,
...
void SubtitleRenderer::draw() BOOST_NOEXCEPT {
  ...
  // font background
  for (size_t i = 0; i < n_lines; ++i)
    draw_text (vg_font_border_, internal_lines_[i], line_positions_[i].first, line_positions_[i].second, 0);

  // font foreground
  for (size_t i = 0; i < n_lines; ++i)
    draw_text (vg_font_, internal_lines_[i], line_positions_[i].first, line_positions_[i].second, white_level_);
